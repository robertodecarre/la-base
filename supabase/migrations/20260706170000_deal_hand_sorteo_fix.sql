-- La Base — fixes a regression introduced by 20260706160000_choose_team_rpc.sql.
--
-- That migration's CREATE OR REPLACE of deal_hand was copied from the
-- pre-sorteo version (20260706020000), which lost the dealer_seat-from-
-- rooms.sorteo_inicial.ganador_seat logic that 20260706150000 had added
-- (deal_hand fell back to a plain floor(random()) for hand 0 again,
-- silently disconnecting the sorteo shown on screen from the actual
-- dealer — caught by online-sorteo-inicial.spec.js asserting the sorteo
-- winner's seat shows "PIE" on the table). This migration restores that
-- logic; the seat-is-not-null readiness gate from 20260706160000 stays.
-- Full body repeated (CREATE OR REPLACE requires it).
