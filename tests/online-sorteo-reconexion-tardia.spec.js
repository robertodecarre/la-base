...
// que vio el tester.
//
// El fix agrega sorteoCumplido: un flag LOCAL a cada sesión que arranca su
// propia ventana de ~3s en cuanto ESA sesión observa tieneSorteo, sin
// importar si gameState ya existía en ese mismo momento. Este test
// reproduce el mount tardío con un reload real a mitad de la mano 0 (no
// las 4 sesiones montando juntas, que es lo que ya cubre
// online-sorteo-inicial.spec.js y no alcanza a agarrar este caso).
test("online: una sesión que recarga después de que la mano 0 ya se repartió igual ve el sorteo", async ({ browser }) => {
  ...
  try {
    ...
    // inicial Y game_state ya existen los dos en la base.
    ...
    // ...y después de su propia ventana de ~3s, pasar a la mesa como
    // cualquier otra sesión — no quedarse trabada mostrándolo para siempre.
